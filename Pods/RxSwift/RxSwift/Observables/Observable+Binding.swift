...
extension ObservableType {
    ...
    public func multicast<S: SubjectType>(_ subject: S)
        ...
        return ConnectableObservableAdapter(source: asObservable(), subject: subject)
    }
...
extension ObservableType {
    ...
    public func shareReplay(_ bufferSize: Int)
        -> Observable<E> {
        if bufferSize == 1 {
            return ShareReplay1(source: asObservable())
        } else {
    ...
    public func shareReplayLatestWhileConnected()
        -> Observable<E> {
        return ShareReplay1WhileConnected(source: asObservable())
    }
}
